...
const { cacheJourneyPlans, getCachedJourneyPlans, invalidateJourneyPlanCache } = require('../lib/redisService');
...
const createJourneyPlan = async (req, res) => {
  try {
    ...
    console.log('Creating journey plan with:', {
      clientId,
      date,
      salesRepId,
      notes,
    ...
    // Create the journey plan with more included relations
    ...
    console.log('Journey plan created successfully:', {
      id: journeyPlan.id,
      date: journeyPlan.date,
      clientName: journeyPlan.client.name,
      routeName: journeyPlan.route?.name
    });
    ...
    res.status(201).json({
      ...
      data: {
        ...journeyPlan,
        statusText: getStatusText(journeyPlan.status)
      }
    });
  } catch (error) {
...
const getJourneyPlans = async (req, res) => {
  ...
  try {
    ...
    const status = req.query.status; // Add status filter support
    ...
    if (status !== undefined) {
      whereClause.status = status === 'in_progress' ? 2 : parseInt(status);
    }
    ...
    const mappedJourneyPlans = journeyPlans.map(plan => ({
      ...plan,
      statusText: getStatusText(plan.status),
      date: plan.date.toISOString(),
      checkInTime: plan.checkInTime?.toISOString(),
      checkoutTime: plan.checkoutTime?.toISOString()
    }));

    // Return paginated result
    const paginatedData = mappedJourneyPlans.slice(skip, skip + limit);
    const elapsed = getElapsedTime(requestStart);
    ...
    const elapsed = getElapsedTime(requestStart);
    console.error(`Error fetching journey plans (${elapsed}ms):`, error);
    
    res.status(error.message === 'SalesRep authentication required' ? 401 : 500).json({
      error: error.message === 'SalesRep authentication required' ? 'Authentication required' : 'Failed to fetch journey plans',
      details: process.env.NODE_ENV === 'development' ? error.message : undefined,
...
const updateJourneyPlan = async (req, res) => {
  ...
  upload(req, res, async (err) => {
    ...
    try {
      ...
      try {
        updatedJourneyPlan = await retryOperation(
          async () => {
            return await withConnectionRetry(async () => {
              return await prisma.$transaction(
                async (tx) => {
                  ...
                  if (
                    clientId &&
                    (checkoutLatitude === undefined || checkoutLongitude === undefined)
                  ) {
                  ...
                  const updated = await tx.journeyPlan.update({
                    ...
                    data: {
                      ...
                      status:
                        status !== undefined ? STATUS_MAP[status] : existingJourneyPlan.status,
                      ...
                      notes: notes,
                      showUpdateLocation:
                        showUpdateLocation !== undefined ? Boolean(showUpdateLocation) : undefined,
                      client: clientId
                        ? {
        ...
        console.error(
          'Transaction failed after retries, using fallback update:',
      ...
      console.log('Journey plan updated successfully:', {
        ...
        checkoutLocation:
          status === 'completed'
            ? `${updatedJourneyPlan.checkoutLatitude}, ${updatedJourneyPlan.checkoutLongitude}`
            : 'N/A',
      });

      // After successful update
      const dateKey = updatedJourneyPlan.date.toISOString().split('T')[0];
      await invalidateJourneyPlanCache(salesRepId, dateKey);
...
const deleteJourneyPlan = async (req, res) => {
  ...
  try {
    ...
    if (journeyPlan.status !== 0) {
      return res
        .status(400)
        .json({ error: 'Only pending journey plans (status 0) can be deleted' });
    }
...
// Simplified fast checkout function
const checkoutJourneyPlan = async (req, res) => {
  ...
  try {
    ...
    console.log('[FAST CHECKOUT] Processing checkout:', {
      journeyId,
      salesRepId,
      checkoutTime,
      checkoutLatitude,
    ...
    if (existingJourneyPlan.userId !== salesRepId) {
      return res.status(403).json({ error: 'Unauthorized to checkout this journey plan' });
    }

    // Validate current status (should be checked_in or in_progress)
    if (existingJourneyPlan.status === 3) {
      return res.status(400).json({ error: 'Journey plan already completed' });
    }

    if (existingJourneyPlan.status === 0) {
      return res.status(400).json({ error: 'Cannot checkout without checking in first' });
    }

    // Prepare checkout data with fallbacks
    const finalCheckoutTime = checkoutTime ? new Date(checkoutTime) : new Date();
    const finalCheckoutLat = checkoutLatitude ? parseFloat(checkoutLatitude) : 0;
    const finalCheckoutLng = checkoutLongitude ? parseFloat(checkoutLongitude) : 0;

    // Fast direct update - only essential checkout fields
    ...
    console.log('[FAST CHECKOUT] Completed successfully:', {
      journeyId: updatedJourneyPlan.id,
      status: 'completed',
      checkoutTime: updatedJourneyPlan.checkoutTime,
      location: `${updatedJourneyPlan.checkoutLatitude}, ${updatedJourneyPlan.checkoutLongitude}`,
      elapsed: `${elapsed}ms`
    });

    // Invalidate cache in background (don't wait)
    const dateKey = new Date().toISOString().split('T')[0];
    invalidateJourneyPlanCache(salesRepId, dateKey).catch(err => 
    ...
    const elapsed = getElapsedTime(requestStart);
    console.error(`[FAST CHECKOUT] Error (${elapsed}ms):`, error);
    
    res.status(500).json({
      success: false,
      error: 'Failed to checkout journey plan',
      details: process.env.NODE_ENV === 'development' ? error.message : undefined,
      timing: {
        total: elapsed
      }
...
module.exports = { createJourneyPlan, getJourneyPlans, updateJourneyPlan, deleteJourneyPlan, checkoutJourneyPlan };
