    ...
    try {
      ...
      for (const item of orderItemsToUse) {
        ...
        const activeStores = product.storeQuantities.filter((sq) => {
          ...
          if (store.status !== 0) {
            console.log(
              `[Order Debug] Store ${store.id} (${store.name}) skipped: inactive (status ${store.status})`
            );
            return false;
          }
        ...
        if (userCountryId === 2) {
          ...
          console.log(
            '[Order Debug] 🎯 Country 2 override activated - restricting to user country only'
          );
          ...
          availableStoreQuantities = activeStores.filter((sq) => {
            ...
            if (matches) {
              console.log(
                `[Order Debug] Country 2 override: Store ${store.id} (${store.name}) matched user country:`,
                {
          ...
          console.log(
            '[Order Debug] Using normal stock validation logic for country:',
            userCountryId
          );
          ...
          console.log('[Order Debug] Region and country matching stores:', {
            ...
            regionStock: regionMatchingStores.reduce(
              (sum, sq) => sum + Number(sq.quantity || 0),
              0
            ),
            countryStock: countryMatchingStores.reduce(
              (sum, sq) => sum + Number(sq.quantity || 0),
              0
            ),
          });
        ...
        const totalAvailableQuantity = availableStoreQuantities.reduce((sum, sq) => {
          ...
          const quantity =
            sq.quantity !== null && sq.quantity !== undefined ? Number(sq.quantity) : 0;

          console.log(
            `[Order Debug] Adding quantity from store ${sq.store.id} (${sq.store.name}):`,
            {
        ...
        if (availableStoreQuantities.length === 0) {
          ...
          const totalActiveStock = activeStores.reduce(
            (sum, sq) => sum + Number(sq.quantity || 0),
            0
          );
        ...
        const maxQuantityStore = availableStoreQuantities.reduce(
          (max, sq) => (sq.quantity > (max?.quantity || 0) ? sq : max),
          null
        );
        ...
        console.log(
          '[Order Debug] Max available quantity for product',
          product.name,
          'in selected store:',
          {
      ...
      try {
        ...
        for (const item of orderItemsToUse) {
          ...
          if (!product) {
            console.log(
              `[Order Debug] Product ${item.productId} not found in batch results, skipping`
            );
            continue;
          }
          ...
          if (item.priceOptionId) {
            ...
            if (priceOption) {
              ...
              console.log(
                `[Order Debug] Price option ${item.priceOptionId} not found for product ${product.id}, using default price`
              );
            }
          }
...
const updateOrder = async (req, res) => {
  ...
  try {
    ...
    await prisma.$transaction(
      ...
      { timeout: 10000 }
    ); // 10 second timeout for the transaction
