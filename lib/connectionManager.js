    ...
    for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
      try {
        ...
        if (error.message.includes('Lock wait timeout') || error.code === 1205) {
          console.warn(
            `Lock timeout detected for ${context}, attempt ${attempt}/${this.maxRetries}`
          );
        ...
        console.warn(
          `Connection attempt ${attempt} failed for ${context}, retrying in ${this.retryDelay}ms:`,
          error.message
        );
  ...
  async executeWithLockTimeoutRetry(operation, context = '', maxRetries = 3) {
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        console.warn(
          `Lock timeout on attempt ${attempt}/${maxRetries} for ${context}, retrying in ${Math.round(delay)}ms`
        );
        await new Promise((resolve) => setTimeout(resolve, delay));
      }
    }
