...
const { withTransaction, withConnectionRetry: withTxConnectionRetry } = require('./transactionManager');
...
class TokenService {
  ...
  async validateToken(token, salesRepId, tokenType = 'access') {
    ...
    try {
      const tokenRecord = await retryWithLockTimeout(
        async () => {
  ...
  async refreshTokens(userId, role) {
    try {
      const result = await withTxConnectionRetry(async () => {
      ...
      }, { context: 'token-refresh-connection' });
  ...
  async generateTokenPair(userId, role) {
    ...
    const refreshToken = jwt.sign({ userId, role, type: 'refresh' }, process.env.JWT_SECRET, {
      expiresIn: '7d',
    });
  ...
  async updateLastUsed(tokenId) {
    ...
    setImmediate(async () => {
      try {
        await withTxConnectionRetry(async () => {
          return await withTransaction(
            async (tx) => {
            ...
            {
              timeout: 5000, // 5 seconds for non-critical operation
              context: 'last-used-update',
              maxRetries: 2,
            }
          );
        }, { context: 'last-used-connection' });
      } catch (error) {
  ...
  async cleanupExpiredTokens(batchSize = 50) {
    // Reduced batch size
    try {
      const now = new Date();
