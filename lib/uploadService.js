...
async function optimizeImage(buffer, mimetype, type = 'document') {
  ...
  try {
    ...
    let optimizer = sharp(buffer).resize(dimensions.width, dimensions.height, {
      fit: 'inside',
...
async function uploadFiles(files, options = {}) {
  ...
  const { results: uploadResults } = await PromisePool.for(files)
...
async function uploadFile(file, options = {}) {
  ...
  const thumbnailUpload = generateThumbnail
    ? optimizeImage(file.buffer, file.mimetype, 'thumbnail').then((thumbBuffer) =>
