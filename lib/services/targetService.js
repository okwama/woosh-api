...
class TargetService {
  async createTestData() {
    ...
    if (!salesRep) {
      throw new Error('No sales rep found. Please create a sales rep first.');
    }
  ...
  async getAllTargetsWithProgress() {
  ...
  async getDailyVisitTargets(userId, date) {
    ...
    const progress =
      salesRep.visits_targets > 0 ? (completedVisits / salesRep.visits_targets) * 100 : 0;
  ...
  async getMonthlyVisitReports(userId, month, year) {
    ...
    for (let d = 1; d <= endDate.getDate(); d++) {
      ...
      const report = {
        ...
        status:
          dayVisits.completedVisits >= user.visits_targets ? 'Target Achieved' : 'In Progress',
      };
